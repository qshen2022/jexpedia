...
export default function FlightBookingPage() {
  ...
  useEffect(() => {
    ...
    apiClient.getFlightById(flightId)
      .then((data) => {
  ...
  const handleStep2Submit = async () => {
    ...
    const result = await apiClient.bookFlight({
      flightId: flight.id,
      passengers,
      seatClass,
    });
    ...
    if (result.success && result.bookingId) {
      ...
      if (result.tripGroupId) {
        setTripGroupId(result.tripGroupId);
      }
      // Fetch arrival airport city for connected trip flow
      try {
        const airportData = await apiClient.getAirportByCode(flight.arrivalAirport);
        if (airportData) {
