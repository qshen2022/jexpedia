...
import type { FlightBookingRow, HotelBookingRow } from "@jexpedia/shared";
...
export default async function MyTripsPage() {
  ...
  const apiClient = createApiClient((session as any).accessToken);
  let flightBookingRows: FlightBookingRow[] = [];
  let hotelBookingRows: HotelBookingRow[] = [];
  try {
    const trips = await apiClient.getMyTrips();
    flightBookingRows = trips.flightBookings;
    hotelBookingRows = trips.hotelBookings;
  } catch {
            ...
            <div key={groupId} className="space-y-2">
              ...
              <div className="space-y-2 pl-2 border-l-2 border-blue-200">
                {group.flights.map((booking) => (
                  <FlightBookingCard key={booking.bookingId} booking={booking} upcoming={upcoming} />
                ))}
                {group.hotels.map((booking) => (
                  <HotelBookingCard key={booking.bookingId} booking={booking} upcoming={upcoming} />
                ))}
              </div>
            </div>
          );
      ...
      {!hasAny && (
        <Card>
          <CardContent className="py-16 text-center space-y-4">
            ...
            <div>
              <h2 className="text-lg font-semibold mb-1">You haven&apos;t booked any trips yet.</h2>
            ...
            <div className="flex items-center justify-between mt-3">
              ...
              {showActions && (
                <div className="flex items-center gap-2">
                  ...
                  />
                  <CancelBookingButton type="flight" bookingId={booking.bookingId} />
                </div>
              )}
      ...
      <CardContent className="p-4">
        <div className="flex items-start gap-3">
          ...
          <div className="flex-1 min-w-0">
            ...
            <p className="text-sm text-muted-foreground mt-0.5">
              {booking.hotelCity}, {booking.hotelCountry} &middot; {booking.roomTypeName}
            </p>
            <p className="text-sm text-muted-foreground mt-0.5">
              ...
              {showActions && (
                <div className="flex items-center gap-2">
                  <ChangeHotelDatesButton
                    ...
                    rooms={booking.rooms}
                  />
                  <CancelBookingButton type="hotel" bookingId={booking.bookingId} />
                </div>
              )}
              {booking.status === "confirmed" && !upcoming && (
                <CancelBookingButton type="hotel" bookingId={booking.bookingId} />
