...
import type { FlightSearchResult, HotelWithRooms } from "@jexpedia/shared";
...
export function ModifyTripDialog({ flight, hotel }: { flight: FlightInfo; hotel: HotelInfo }) {
  ...
  const [hotelResults, setHotelResults] = useState<HotelWithRooms[]>([]);
  ...
  async function handleSelectFlight(f: FlightSearchResult) {
    setSelectedFlight(f);
    setLoading(true); setError("");
    try {
      const arrivalDate = f.arrivalTime.split("T")[0];
      const data = await apiClient.searchHotels({
  ...
  function handleSelectRoom(h: HotelWithRooms, room: { id: string; name: string; pricePerNight: number }) {
    setSelectedHotel(h);
