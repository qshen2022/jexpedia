...
import { modifyFlightDates } from "@/lib/actions/booking";
...
export function ChangeFlightDatesButton({
  ...
  const handleSearch = async () => {
    ...
    try {
      const res = await fetch(
        `/api/flights/search?from=${encodeURIComponent(departureAirport)}&to=${encodeURIComponent(arrivalAirport)}&date=${encodeURIComponent(newDate)}&pax=${pax}&class=${encodeURIComponent(seatClass)}`
      );
      if (!res.ok) {
  ...
  const handleSelect = async (flightId: string) => {
    ...
    const result = await modifyFlightDates(bookingId, flightId);
  ...
  const getPrice = (flight: FlightResult) =>
